...
class Fahrt():
    ...
    def loop(self):
        ...
        if int(getTime.split(":")[0]) <= 0 and int(getTime.split(":")[1]) <= 0 and int(getTime.split(":")[2]) <= 0:
            ...
            print("")
            print("[*] Start time: " + self.start + ":00")
            print("[*] End time:   " + self.end + ":00")
            self.done()
            self.duration()
            print("")
            self.progress()
    ...
    def done(self):
        time_done = self.now_total_secs - self.start_total_secs

        print("[*] Time done:  " + self.toTime(time_done))

    def duration(self):
        time_duration = self.end_total_secs - self.start_total_secs

        print("[*] Duration:   " + self.toTime(time_duration))
